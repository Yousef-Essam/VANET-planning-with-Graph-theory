...
clear avgconjestion;
clear avgenergy;
clear avgconnectivity

[motionmatx, motionmaty, xdat, ydat] = Pathdata(g, s, x, y, spd, lod);
avgconjestion = zeros(0, 0);
avgenergy = zeros(0, 0);
avgconnectivity = zeros(0, 0);
...
for ii = 1:trials
    ...
    %%
        ...
        [energy, conjestion, connectivity] = results(Cg, n, ext, energy, ...
            conjestion, connectivity);
        avgenergy(k) = energy / k;
        avgconjestion(k) = conjestion / k;
        avgconnectivity(k) = connectivity / k;
        
        pause(.01);                                                    % to have a simulation with a suitable and remarkable speed not with the speed of the processor 
        if k == 10 && save == 1
           saveas(gcf, ['map' int2str(map) ' top' int2str(topology) '.png']) 
        end
...
plotting(avgenergy, avgconnectivity, avgconjestion, map, topology, save);      % map - top - save